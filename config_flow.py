...
from .const import DOMAIN, CONF_EMAIL, CONF_PASSWORD, CONF_REGION, CONF_GEMINI_API_KEY, REGIONS, VEHICLE_SPECS
...
class VinFastConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            # Đặt ID duy nhất là Email để tránh cài trùng lặp
            await self.async_set_unique_id(user_input[CONF_EMAIL].lower())
            self._abort_if_unique_id_configured()
            
            # Tiêu đề Integration trên UI sẽ là Email của người dùng
            return self.async_create_entry(title=user_input[CONF_EMAIL], data=user_input)

        # Lấy danh sách tên khu vực từ file const để hiển thị lên UI
        region_options = {key: val["name"] for key, val in REGIONS.items()}

        # Form cài đặt lần đầu
        data_schema = vol.Schema({
            vol.Required(CONF_REGION, default="vn"): vol.In(region_options),
...
class VinFastOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        # Cố gắng lấy thông tin dòng xe hiện tại để điền gợi ý (fallback) chuẩn xác
        ...
        # Lấy dữ liệu cũ nếu đã từng lưu, nếu chưa có thì dùng mặc định
        ...
        options_schema = vol.Schema({
            ...
            vol.Required("cost_per_kwh", default=cost_per_kwh): int,
            vol.Required("gas_price", default=gas_price): int,
            vol.Required("ev_kwh_per_km", default=ev_kwh_per_km): float,
            vol.Required("gas_km_per_liter", default=gas_km_per_liter): float,
        })

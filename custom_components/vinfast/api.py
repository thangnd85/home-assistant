...
class VinFastAPI:
    ...
    def __init__(self, email, password, vin=None, vehicle_name="Xe VinFast", options=None, gemini_api_key=""):
        ...
        self._last_data = {
            ...
            "api_ai_advisor": "Hệ thống AI đang chờ hoàn thành chuyến đi đầu tiên để phân tích..."
        }  
    ...
    def _safe_request(self, method, url, max_retries=6, delay=10, **kwargs):
        for attempt in range(max_retries):
            try:
                ...
                _LOGGER.warning(f"VinFast Network: Doi mang (Thu lai {attempt+1}/{max_retries})")
    ...
    def _delayed_ping(self):
        time.sleep(3)
        self._send_app_ping_only()
        time.sleep(5)
        self._send_app_ping_only()
    ...
    def _get_active_sensors_list(self):
        ...
        elif any(m in model for m in ["VF 5", "VF 6", "VF 7", "VF5", "VF6", "VF7"]): 
    ...
    def fetch_charging_history(self):
        try:
            ...
            if is_success:
                ...
                if total_records_from_api > 0: new_t_sessions = total_records_from_api
                else: new_t_sessions = len(valid_sessions)
                ...
                if new_t_sessions > 0 and new_t_sessions >= current_t_sessions: self._last_data["api_total_charge_sessions"] = new_t_sessions
                elif current_t_sessions == 0 and new_t_sessions == 0: self._last_data["api_total_charge_sessions"] = 0

                home_kwh = safe_float(self._last_data.get("api_home_charge_kwh", 0.0))
                final_total_kwh = public_energy + home_kwh
                self._last_data["api_total_energy_charged"] = round(final_total_kwh, 2)
                ...
                if new_t_sessions == 0 and current_t_sessions > 0: threading.Thread(target=self._retry_fetch_charging_history, daemon=True).start()
                else:
                    self._calculate_advanced_stats()
                    self._save_state()
                    if self.callbacks:
    ...
    def _api_polling_loop(self):
        ...
        while self._running:
            try:
                ...
                if now - getattr(self, '_last_mqtt_msg_time', now) > 600:
                    self._renew_aws_connection()
                    self._last_mqtt_msg_time = now
                    
                if now - last_aws_renew >= 7200:
                    last_aws_renew = now
                    self._renew_aws_connection()
                ...
                if getattr(self, '_is_moving', False):
                    pass 
                elif getattr(self, '_is_charging', False):
                    if now - last_app_ping >= 300: 
                        last_app_ping = now
                        self._send_app_ping_only()
                else:
                    if now - last_app_ping >= 1800: 
                        last_app_ping = now
                        self._send_app_ping_only()
            ...
            if data_dict:
                ...
                if self._is_charging and not getattr(self, '_last_is_charging', False):
                    ...
                    if delta_soc > 0.5: 
                        ...
                        if getattr(self, '_current_charge_max_power', 0.0) < 5.0:
                ...
                self._is_moving = (speed > 0) or (str(gear) in ["2", "4", "D", "R"]) 
                        ...
                        if getattr(self, '_is_trip_active', False):
                            ...
                            if not self._route_coords:
                                ...
                                if is_valid_point and (distance_m >= 1.0 or final_lat != lat_f):
                                    ...
                                    gps_trip_dist = self._trip_accumulated_distance_m / 1000.0
                                    self._last_data["api_trip_distance"] = round(gps_trip_dist, 2)
                                    ...
                                    if gps_trip_dist > 0:
                                        if self.gas_km_per_liter > 0:
                                            self._last_data["api_trip_gas_cost"] = round((gps_trip_dist / self.gas_km_per_liter) * self.gas_price, 0)
                                        self._last_data["api_trip_charge_cost"] = round(gps_trip_dist * self.ev_kwh_per_km * self.cost_per_kwh, 0)
                                        
                                        if self._trip_start_time and self._trip_start_time > 0:
                                            trip_hrs = (current_time - self._trip_start_time) / 3600.0
                                            if trip_hrs > 0:
                                                self._last_data["api_trip_avg_speed"] = round(gps_trip_dist / trip_hrs, 1)
                                        
                                        if getattr(self, '_trip_start_soc', None) is not None and self._trip_start_soc >= current_soc:
                                            cap = safe_float(self._last_data.get("api_static_capacity", 0))
                                            if cap > 0:
                                                energy_used = ((self._trip_start_soc - current_soc) / 100.0) * cap
                                                self._last_data["api_trip_energy_used"] = round(energy_used, 2)
                                                self._last_data["api_trip_efficiency"] = round((energy_used / gps_trip_dist) * 100, 2)

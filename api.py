...
class VinFastAPI:
    def __init__(self, email, password, vin=None, vehicle_name="Xe VinFast", options=None):
        ...
        # Biến thông tin Trip để lưu History
        ...
        self._known_keys = set(BASE_SENSORS.keys()).union(VF3_SENSORS.keys()).union(VF567_SENSORS.keys()).union(VF89_SENSORS.keys())
    ...
    def _post_api(self, path, payload, max_retries=1, vin_override=None):
        for _ in range(max_retries + 1):
            ...
            headers.update({"X-HASH": self._generate_x_hash("POST", path, request_vin, ts), "X-HASH-2": self._generate_x_hash_2("android", request_vin, DEVICE_ID, path, "POST", ts), "X-TIMESTAMP": str(ts)})
            try:
    ...
    def _delayed_fetch_charging_history(self):
        time.sleep(60); self.fetch_charging_history()
    ...
    def _renew_aws_connection(self):
        try:
            self.login(); self._register_device_trust(); new_url = self._get_aws_mqtt_url()
            if self.client:
                self.client.loop_stop(); self.client.disconnect()
                self.client.ws_set_options(path=new_url.split(IOT_ENDPOINT)[1])
                self.client.connect(IOT_ENDPOINT, 443, 60); self.client.loop_start(); self.register_resources()
        except Exception: pass

    def _api_polling_loop(self):
        start_time = time.time()
        last_heartbeat, last_charge_fetch, last_token_renew, last_app_sim = start_time, start_time, start_time, start_time
        time.sleep(3) 
        if self.client:
            self.register_resources(); self.fetch_charging_history(); self.fetch_nearby_stations() 

        while self._running:
            try:
                time.sleep(1); now = time.time()
                if now - last_heartbeat >= 120: last_heartbeat = now; self._send_heartbeat("1")
                if now - last_charge_fetch >= 3600: last_charge_fetch = now; self.fetch_charging_history()
                if now - last_token_renew >= 3000: last_token_renew = now; self._renew_aws_connection()
                if now - last_app_sim >= 900:
                    last_app_sim = now
                    if not self._is_moving and not self._is_charging: self.register_resources()
                if getattr(self, '_force_full_scan', False):
                    self._force_full_scan = False; last_app_sim = now; self.register_resources()
            except Exception: pass
    ...
    def start_mqtt(self):
        ...
        self.client.tls_set(); self.client.connect(IOT_ENDPOINT, 443, 60); self.client.loop_start()
        self._polling_thread = threading.Thread(target=self._api_polling_loop, daemon=True); self._polling_thread.start()

    def _on_connect(self, client, userdata, flags, rc, properties=None):
        if rc == 0: 
            client.subscribe(f"/mobile/{self.vin}/push", qos=1); client.subscribe(f"monitoring/server/{self.vin}/push", qos=1); client.subscribe(f"/server/{self.vin}/remctrl", qos=1)
            self._send_heartbeat("2")

    def _on_disconnect(self, client, userdata, rc): pass

    # =========================================================
    # HÀM LƯU LỊCH SỬ CHUYẾN ĐI VÀO Ổ CỨNG HA (Bypass 16KB Limit)
    # =========================================================
    def _save_trip_to_history(self):
        try:
            ...
            history_file = f"{www_dir}/vinfast_trips_{self.vin.lower()}.json"
            trips = []
            ...
            end_dt = datetime.datetime.fromtimestamp(time.time())
            duration_mins = int((time.time() - self._trip_start_time) / 60)
            ...
            # Lưu tối đa 30 chuyến đi gần nhất để không nặng thẻ nhớ
            trips.insert(0, new_trip) 
            if len(trips) > 30: trips = trips[:30]
            
            with open(history_file, 'w', encoding='utf-8') as f:
                json.dump(trips, f, ensure_ascii=False)
            _LOGGER.info(f"VinFast: Đã lưu thành công lịch sử Trip ngày {new_trip['date']}")
    ...
    def _on_message(self, client, userdata, msg):
        try:
            ...
            if data_dict:
                ...
                if str(data_dict.get("34183_00001_00055", "0")) == "1": self._send_heartbeat("1")

                time_since_last = current_time - getattr(self, '_last_activity_time', current_time)
                self._last_activity_time = current_time
                if not self._is_moving and not self._is_charging and time_since_last > 180: self._force_full_scan = True
                ...
                if self._is_charging and not self._last_is_charging:
                    self._last_data["api_last_charge_start_soc"] = current_soc; self._last_is_charging = True
                elif not self._is_charging and self._last_is_charging:
                    self._last_data["api_last_charge_end_soc"] = current_soc; self._last_is_charging = False
                ...
                # FIX LỖI XE KHÔNG CÓ SỐ P (Phanh tay cơ): 
                # 1=P, 2=R, 3=N, 4=D. Chỉ di chuyển khi có tốc độ HOẶC đang cài số D/R
                ...
                if self._is_moving: self._last_move_time = current_time
                
                # SỰ KIỆN KẾT THÚC CHUYẾN ĐI VÀ LƯU FILE
                if not self._is_moving and (current_time - getattr(self, '_last_move_time', current_time) > 1800):
                    if getattr(self, '_is_trip_active', False):
                        self._is_trip_active = False
                        self._save_trip_to_history() # Gọi hàm lưu file Json
                ...
                if self._is_moving and not getattr(self, '_is_trip_active', False) and odo > 0:
                    ...
                    self._last_data["api_trip_distance"] = 0.0; self._last_data["api_trip_avg_speed"] = 0.0

...
class VinFastSensor(SensorEntity):
    def __init__(self, api, device_key, name, unit, icon, dev_class):
        ...
        # BẢO VỆ STATE CLASS ĐỂ KHÔNG BỊ HOME ASSISTANT CẢNH BÁO
        self._attr_state_class = None
        if unit:
            if dev_class == "energy":
                self._attr_state_class = SensorStateClass.TOTAL_INCREASING if "tổng" in name.lower() else SensorStateClass.TOTAL
            elif dev_class == "monetary":
                self._attr_state_class = SensorStateClass.TOTAL
            elif dev_class == "distance":
                if "tổng" in name.lower() or "odo" in name.lower():
    ...
    def extra_state_attributes(self):
        if self._device_key == "api_best_efficiency_band":
            ...
            return attrs if attrs else {"Trạng thái": "Đang phân tích dải tốc độ..."}

        # NHÓM ATTRIBUTE CHO BẢN ĐỒ VÀ TRẠM SẠC
        ...
        if self._device_key == "api_debug_info":
            try: return json.loads(self.api._last_data.get("api_debug_raw", "{}"))
            except: return {"Lỗi": "Không thể parse Raw JSON"}
    ...
    def process_new_data(self, data):
        if self._device_key in data:
            ...
            if self._device_key in ["api_total_charge_cost", "api_total_charge_cost_est", "api_trip_charge_cost", "api_total_gas_cost", "api_trip_gas_cost"]:
                ...
                try:
                    num_val = float(val)
                    if num_val > 10: val = round(num_val / 100, 1)
                    else: val = num_val
                except: pass
